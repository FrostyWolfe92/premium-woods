...
const SPECIES = [
  { name: "Black Walnut",     img: IMG.walnutTex,   note: "Premium hardwood. Dark, rich, classic." },
  { name: "Hard Maple",       img: IMG.mapleTex,    note: "Bright, tight grain. Maximum durability." },
  { name: "American Cherry",  img: IMG.cherryTex,   note: "Warm tones that deepen with age." },
  { name: "Red Oak",          img: IMG.oakTex,      note: "Bold open grain. Midwest workhorse." },
];
  ...
  return (
    <>
      <PageHero
        ...
        bgImage={IMG.kitchenIsland}
      />

      <section className="mx-auto max-w-7xl px-6 py-20 grid lg:grid-cols-2 gap-14 items-start">
        <div>
          <div className="eyebrow">Why Wood</div>
          <h2 className="mt-3 font-display text-4xl font-semibold text-[var(--color-walnut)]">A counter that gets <em className="text-[var(--color-amber-deep)] not-italic">better</em> with use.</h2>
        ...
        <div>
          ...
          <Stagger className="mt-6 grid grid-cols-2 gap-4" gap={0.06}>
            {SPECIES.map((s) => (
              <StaggerItem key={s.name}>
                <div className="rounded-lg overflow-hidden border border-[var(--color-line)] bg-white hover:shadow-lg transition">
